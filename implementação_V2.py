...
class R2AQlearning(IR2A):
  def __init__(self, id):
    ...
    self.tau = 1.0    # Temperatura para Softmax
  ...
  def update_q_table(state, action, reward, next_state):
      if next_state not in self.Q_table:
          self.Q_table[next_state] = np.zeros(self.num_qualities) # Inicializa Q(s, a) se não existir, cria um array de 20 zeros no dicionário Q_table com chave = state.
