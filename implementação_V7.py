...
class R2AQlearning(IR2A):
  ...
  def handle_segment_size_request(self,msg):
    ...
    if self.seg_num == 0:
      ...
      for episode in range(100000):  # Número de iterações de aprendizado
          ...
          bandwidth = random.choice(self.qi)  # Simulação de largura de banda - ESTRATEGIA (AGRUPAR 5 EM 5)
          ...
          # lógica para medir recompensa R

          #RECOMPENSA QUALIDADE!
          RQ = ((((quality - 1)/ num_qualities - 1) * 2) - 1)

          #RECOMPENSA OSCILAÇÃO!
          if osc_length != 0 and osc_depth != 0:
    ...
    #suposição que len(Buffer_filling_lista) e len(quality_lista) == número de segmentos que foram reproduzidos até agr
      ...
      bufferfilling = 5  # Preenchimento do buffer inicial
      buffer_change = 0  # Variação do buffer inicial
      quality = 0  # Qualidade atual ESTRATÉGIA mudar valor p/ um mais alto
      bandwidth_referencial = self.throughputs[0]  # largura de banda inicial (a obtida no response do arquivo MPD) MUDAR AQUI
      osc_length = 0  # Comprimento da oscilação inicial
      osc_depth = 0  # Profundidade da oscilação inicial
      ...
      probabilities = exp_q / np.sum(exp_q)
      action = np.random.choice(range(num_qualities), p=probabilities) #seleciona através da política softmax
      msg.add_quality_id(self.qi[action]) #seleciona a primeira qualidade
      self.quality_lista_1.append(action) # adiciona a qualidade selecionada na lista de históricos de qualidade
  ...
  def handle_segment_size_response(self,msg):
    ...
    #CONSEGUE AS ESTATISTICAS NA MARRA
    ...
    if len(buffer_filling_lista) >= 2:
      ...
      buffer_change = 0
    quality_lista = self.whiteboard.get_playback_qi() #n sei se usa!
    quality = self.quality_lista_1[-1] #verificar DPS self.qi.index(quality_lista[-1][1])
    bandwidth_referencial = self.throughputs[self.seg_num] #MUDAR AQUI
    if bandwidth_referencial <= self.qi[0]:
      bandwidth = self.qi[0]
    elif bandwidth_referencial >= self.qi[len(self.qi)-1]:
      bandwidth = self.qi[len(self.qi)-1]
